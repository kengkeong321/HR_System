    ...
    @push('scripts')
    <div id="payroll-data" data-breakdown='{!! json_encode(is_string($payroll->breakdown) ? json_decode($payroll->breakdown) : ($payroll->breakdown ?? [])) !!}' style="display:none;"></div>
    ...
    <script>
        (function() {
            ...
            function calculateNet() {
                ...
                const allowanceEl = document.getElementById('total_allowances');
                const deductionEl = document.getElementById('total_deductions');
                const dataEl = document.getElementById('payroll-data');

                if (!basicEl || !dataEl) return;

                const basic = parseFloat(basicEl.value) || 0;
                const allowances = parseFloat(allowanceEl.value) || 0;
                const manualDeductions = parseFloat(deductionEl.value) || 0;

                let breakdown = {};
                try {
                    breakdown = JSON.parse(dataEl.getAttribute('data-breakdown')) || {};
                } catch (e) {
                    console.error("Failed to parse payroll breakdown", e);
                }

                const amounts = breakdown.calculated_amounts || {};

                const epf = parseFloat(amounts.epf_employee_rm) || 0;
                const socso = parseFloat(amounts.socso_employee_rm) || 0;
                const eis = parseFloat(amounts.eis_employee_rm) || 0;
