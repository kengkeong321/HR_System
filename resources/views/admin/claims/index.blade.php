    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
            ...
            <div class="btn-group shadow-sm">
                <a href="{{ route('admin.claims.index', ['status' => 'Pending']) }}" class="btn btn-sm {{ $status == 'Pending' ? 'btn-primary' : 'btn-outline-secondary' }}">Pending</a>
                <a href="{{ route('admin.claims.index', ['status' => 'Approved']) }}" class="btn btn-sm {{ $status == 'Approved' ? 'btn-success' : 'btn-outline-secondary' }}">Approved</a>
                <a href="{{ route('admin.claims.index', ['status' => 'Rejected']) }}" class="btn btn-sm {{ $status == 'Rejected' ? 'btn-danger' : 'btn-outline-secondary' }}">Rejected</a>
            </div>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="max-width: 250px;">
                                ...
                                <div class="text-truncate text-secondary" title="{{ $claim->description }}">{{ $claim->description }}</div>
                            </td>
                            ...
                            <td class="text-center pe-4">
                                ...
                                @if($claim->status === 'Pending' && $canAction)
                                ...
                                @elseif(!$canAction && $claim->status === 'Pending')
                                    <span class="badge bg-secondary">View Only</span>
                                @else
                                    {{-- Status Badges for non-pending or completed items --}}
                            ...
                            <div class="modal-dialog modal-xl modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header bg-light">
                                        <h5 class="modal-title"><i class="bi bi-search text-primary me-2"></i>Verify Claim #{{ $claim->id }}</h5>
                                        ...
                                        <div class="row g-0" style="min-height: 500px;">
                                            ...
                                            <div class="col-lg-7 bg-dark d-flex align-items-center justify-content-center" style="min-height: 500px;">
                                                @if($claim->receipt_path)
                                                    ...
                                                    @endif
                                                @else
                                                    <div class="text-white opacity-50 text-center"><p>No Receipt File Found</p></div>
                                                @endif
                                            </div>
                                ...
                                <div class="modal-content border-danger border-top border-4">
                                    <form action="{{ route('admin.claims.reject', $claim->id) }}" method="POST">
                                        ...
                                        <div class="modal-body">
                                            <p class="mb-3">Why is this claim being rejected?</p>
                        ...
                        @empty
                        <tr><td colspan="6" class="text-center py-5 text-muted">No {{ strtolower($status) }} claims found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            @if($claims->hasPages())
            <div class="px-4 py-3 border-top">{{ $claims->appends(['status' => $status])->links() }}</div>
            @endif
        </div>
    </div>
</div>
