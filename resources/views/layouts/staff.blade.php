...
<head>
  ...
  <style>
    ...
    .sidebar {
      width: 240px; transition: width .2s; position: sticky;
      top: 56px; height: calc(100vh - 56px); overflow-y: auto;
      background: white; border-right: 1px solid #dee2e6;
    }
    .sidebar .nav-link { color: #333; padding: 10px 20px; }
    .sidebar .nav-link.active { background-color: #0d6efd; color: white !important; rounded: 5px; }
    .sidebar-collapsed .sidebar { width: 64px; }
    .sidebar-collapsed .sidebar .label, .sidebar-collapsed .sidebar h5 { display: none; }
    
    /* Flash Message Design */
    .flash-message {
      position: fixed; top: -80px; left: 50%; transform: translateX(-50%);
      z-index: 1050; transition: top .4s ease, opacity .4s ease; opacity: 0;
    }
